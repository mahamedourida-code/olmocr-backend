...
async def process_batch_images_background_async(
    ...
    try:
        # Initialize Redis for WebSocket pub/sub AND job tracking
        try:
            redis_service = RedisService()
            await redis_service.initialize()
            logger.info(f"Redis service initialized for WebSocket updates")

            # Create job entry in Redis for status tracking
            job_data = {
                'job_id': job_id,
                'session_id': session_id,
                'user_id': batch_context.get('user_id'),
                'status': 'processing',
                'total_images': len(images),
                'processed_images': 0,
                'progress': 0,
                'output_format': batch_context.get('output_format', 'xlsx'),
            ...
            logger.info(f"Created Redis job entry for {job_id}")
        except Exception as e:
            ...
            try:
                ...
                if redis_service:
                    try:
                        ...
                        logger.warning(f"Failed to update job progress in Redis: {update_error}")
                ...
                # Update Redis job progress even for failed images
                if redis_service:
                    try:
                        progress_pct = int(((i + 1) / len(images)) * 100)
                        await redis_service.update_job(job_id, {
        ...
        # Send completion message via WebSocket AND update Redis job
        if redis_service:
            try:
                ...
                # Update Redis job with completion data
                await redis_service.update_job(job_id, {
                    'status': status,
        ...
        # Send error message via WebSocket AND update Redis job
        if redis_service:
            try:
                # Update Redis job with error status
                await redis_service.update_job(job_id, {
                    'status': 'failed',
                    'error': str(e),
                    'completed_at': datetime.utcnow().isoformat()

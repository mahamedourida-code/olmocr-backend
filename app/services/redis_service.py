...
class RedisService:
    ...
    async def initialize(self) -> None:
        ...
        # List of hosts to try (Windows networking can be tricky)
        hosts_to_try = [
            "localhost",      # Try localhost first
            "127.0.0.1",     # Then 127.0.0.1
            "0.0.0.0",       # Then 0.0.0.0 (if Redis bound to all interfaces)
        ]
        ...
        for host in hosts_to_try:
            try:
                ...
                logger.info(f"Redis connection attempt {self._connection_attempts} using host: {host}")
                
                # Try connection with current host
                self.client, self.pool = await self._try_connection_with_host(host, 6379)
                self._is_connected = True
                logger.info(f"Redis connection established successfully with {host}:6379")
                return
                
            except Exception as e:
                logger.warning(f"Failed to connect to Redis at {host}:6379: {e}")
                last_error = e
        ...
        self._is_connected = False
        raise ConnectionError(f"Failed to connect to Redis after trying {len(hosts_to_try)} hosts. Last error: {last_error}")
